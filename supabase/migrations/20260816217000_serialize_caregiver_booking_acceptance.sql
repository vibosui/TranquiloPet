...
begin
  ...
  select exists(
    ...
    where hep.event_id = p_event_id
      and coalesce(hep.plan_snapshot->>'legacy_unenforced', 'true') <> 'true'
  ) into is_plan_event;

  if p_target_status = 'accepted' and is_plan_event then
    perform 1 from public.profiles p where p.id = event_row.caregiver_id for update;
  ...
  if p_target_status = 'completed' and is_plan_event and exists (
